...
int duplicateReverse(ListNode *cur,ListNode **ptrNewHead)
{
    ...
    while(next != NULL)
    {   
        //Set the first Node point to empty , store next node to somewhere first
        if(count == 0)
        {
            next = temp->next;
            temp->next = NULL;
        ...
        //The next node point to the previous node
        else
        {   
            next = temp->next;
            temp->next = pre;
        ...
        //Store temp as previous node, Update temp to the next node
        pre = temp;
        temp = next;
    }
