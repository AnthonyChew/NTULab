...
{
    int temp = Hash(key, Q3Hash.hSize);
    ListNode *tableTemp = Q3Hash.Table[temp].head;

    while (tableTemp != NULL)
    {
        if(tableTemp->key == key)
        {
            return tableTemp;
        }
...
int HashInsert(HashTable *Q3HashPtr, int key)
{
    ...
    int temp = Hash(key, Q3HashPtr->hSize);

    ListNode *newNode = (ListNode *)malloc(sizeof(ListNode));
    newNode->key = key;
    newNode->next = NULL;

    if (Q3HashPtr->Table[temp].size == 0)
    {   
        Q3HashPtr->Table[temp].head = newNode;
    }
    else
    {
        newNode->next = Q3HashPtr->Table[temp].head;
        Q3HashPtr->Table[temp].head = newNode;
    }

    Q3HashPtr->nSize += 1;
    Q3HashPtr->Table[temp].size += 1;
}
