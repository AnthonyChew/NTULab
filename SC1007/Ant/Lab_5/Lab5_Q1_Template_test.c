...
int HashInsert(HashTable* Q3HashPtr, int key)
{
    ...
    printf("test : %d" , Q3HashPtr[temp].Table->size);

    if(Q3HashPtr[temp].Table->size == 0)
    {
        Q3HashPtr[temp].Table->head = newNode;
    }
    else
    {
        Q3HashPtr[temp].Table->head->next = Q3HashPtr[temp].Table->head;
        Q3HashPtr[temp].Table->head = newNode;
    }
}
